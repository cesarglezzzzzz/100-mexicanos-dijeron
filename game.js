...
const renderQuestionUI = () => {
    ...
    currentQuestion.answers.forEach((ans, i) => {
        ...
        div.className = 'answer-item hidden';
        div.dataset.index = i;
...
const revealAnswer = (index) => {
    ...
    roundPoints += Number(currentQuestion.answers[index].score);

    play(correctSound);

    if(revealed.every(r => r)) concludeRound(currentTeam);
};
...
setupForm.addEventListener('submit', e=>{
    ...
    scores = [0,0]; currentRound=1;
    resetRoundState(); updateScoreUI();
...
restartBtn.addEventListener('click', ()=>{scores=[0,0]; updateScoreUI(); showScreen('welcome-screen'); team1Input.value=''; team2Input.value='';});
